...
function get_home_temp() {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("home_temp").innerHTML = 
      this.responseText;	  
    }
  };
...
function get_home_humidity() {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("home_humidity").innerHTML =
      this.responseText;
    }
  };
  xhttp.open("GET", "/read_humidity", true);
  xhttp.send();
}

function get_home_pressure() {
  var xhttp = new XMLHttpRequest();
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("home_pressure").innerHTML =
      this.responseText;
    }
  };
  xhttp.open("GET", "/read_pressure", true);
  xhttp.send();
}
